...
public struct ConfettiCannon: View {
    ...
    public var body: some View {
        ZStack {
            ForEach(finishedAnimationCouter..<animate.count, id:\.self) { _ in
                ConfettiContainer(
        ...
        .onChange(of: counter) { value in
            if firtAppear {
                for iii in 0...confettiConfig.repetitions {
                    DispatchQueue.main.asyncAfter(deadline: .now() + confettiConfig.repetitionInterval * Double(iii)) {
...
class ConfettiConfig: ObservableObject {
    internal init(num: Int, shapes: [AnyView], colors: [Color], confettiSize: CGFloat,
                  rainHeight: CGFloat, fadesOut: Bool, opacity: Double, openingAngle:Angle,
                  closingAngle:Angle, radius:CGFloat, repetitions:Int, repetitionInterval:Double) {
        self.num = num
        self.shapes = shapes
        self.colors = colors
