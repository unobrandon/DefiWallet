    ...
    func body(content: Content) -> some View {
        return GeometryReader { proxy in
            content
                ...
                .backgroundPreferenceValue(Refresh.HeaderAnchorKey.self) { vvv -> Color in
                    DispatchQueue.main.async { self.update(proxy: proxy, value: vvv) }
                    return Color.clear
                }
                .backgroundPreferenceValue(Refresh.FooterAnchorKey.self) { vvv -> Color in
                    DispatchQueue.main.async { self.update(proxy: proxy, value: vvv) }
                    return Color.clear
                }
                .id(self.id)
