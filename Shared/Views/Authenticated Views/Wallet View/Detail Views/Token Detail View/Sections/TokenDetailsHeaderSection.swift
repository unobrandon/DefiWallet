    ...
    func detailsHeaderSection() -> some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .center, spacing: 10) {
                RemoteImage(tokenDescriptor?.imageLarge ?? tokenDetail?.image, size: 48)
                    .clipShape(Circle())
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Text("#\(tokenDescriptor?.marketCapRank ?? 0) " + (tokenDescriptor?.symbol?.uppercased() ?? tokenDetail?.symbol?.uppercased() ?? ""))
            ...
            HStack(alignment: .center, spacing: 5) {
                ...
                if let priceChange = tokenDetail?.priceChange24H,
