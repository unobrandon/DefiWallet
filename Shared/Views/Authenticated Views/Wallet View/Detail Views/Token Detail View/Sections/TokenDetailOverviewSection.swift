                ...
                LazyVGrid(columns: gridItems, alignment: .center, spacing: 5) {
                    ...
                    if let marketCap = tokenModel?.marketCap ?? tokenDetails?.marketCap,
                       let change = tokenModel?.marketCapChangePercentage24H {
                        StackedStatisticLabel(title: "Market Cap",
                                              metric: "\(Locale.current.currencySymbol ?? "")\("".formatLargeNumber(Int(marketCap), size: .regular).capitalized)", number: nil, percent: "\(change >= 0 ? "+" : "")\(change.reduceScale(to: 3))%", percentColor: change >= 0 ? .green : .red, style: service.themeStyle)
                    }
                    ...
                    if let fullyDilutedValuation = tokenModel?.fullyDilutedValuation ?? tokenDetails?.fullyDilutedValuation {
                        StackedStatisticLabel(title: "Fully Diluted \nValuation",
                                              metric: Int(fullyDilutedValuation) >= 99999999 ? "\(Locale.current.currencySymbol ?? "")" + "\("".formatLargeNumber(Int(fullyDilutedValuation), size: .regular).capitalized)" : "\(Locale.current.currencySymbol ?? "")" + fullyDilutedValuation.formatCommas())
                    } else {
                        StackedStatisticLabel(title: "Fully Diluted \nValuation", metric: "-")
                    }
